...
const create = (connection, data) => {
  return new Promise((resolve, reject) => {
    connection.query(`insert into pessoas (nome, nascimento, sexo, cargo) values('${data.nome}','${data.nascimento}','${data.sexo}','${data.cargo}')`, (err) => {
      if(err){
        reject(err)
      }else{
        resolve();
...
const update = (connection, id, data) => {
  return new Promise((resolve, reject) => {
    connection.query(`update pessoas set nome='${data.nome}', nascimento='${data.nascimento}', sexo='${data.sexo}', cargo='${data.cargo}' where id=${id}`, (err) => {
      if(err){
        reject(err)
      }else{
        resolve();
